...
async function test(){
  ...
  await Promise.all([vm.runInContext('UpdateRulesPatch({X:{Enabled:false}})',worker),vm.runInContext('UpdateRulesPatch({TikTok:{DailyLimitMinutes:45}})',worker)]);
  assert.equal(stored.X.Enabled,false,'Concurrent updates preserve the disabled app');
