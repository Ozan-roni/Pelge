...
async function run(){
 ...
 try{
  ...
  for(const width of [320,390,430])for(const theme of ['light','dark']){
   ...
   await page.evaluate(()=>{window.cameraClicks=0;document.querySelector('#native-camera').onclick=()=>window.cameraClicks++;document.querySelector('.person').onclick=()=>{document.querySelector('#private-conversation').innerHTML='<textarea aria-label="Native message"></textarea>';};});
   ...
   assert.equal(await page.evaluate(()=>window.cameraClicks),0,'Switching views must not access camera or send anything');
   await page.locator('#native-camera').click();assert.equal(await page.evaluate(()=>window.cameraClicks),1,'Native camera listener retained');
   ...
   await page.getByRole('button',{name:'Messages',exact:true}).click();await page.locator('.rail').waitFor({state:'visible'});
   assert.equal(await page.locator('.camera').isVisible(),false);
